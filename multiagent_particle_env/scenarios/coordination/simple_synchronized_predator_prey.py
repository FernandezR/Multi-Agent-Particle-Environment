...
"""
simple_synchronized_predator_prey.py

Synchronized Predator-prey

Communication: Yes, but not with communication action
Competitive:   Yes
...
from multiagent_particle_env.utils import dict2namedtuple
...
class Scenario(BaseScenario):
    ...
    def make_world(self, args):
        ...
        world.synchronized_predator_prey = True
        ...
        self.scale_miscapture_punishment = getattr(args, "scale_miscapture_punishment", False)
        ...
        if self.scale_miscapture_punishment:
            self.miscapture_punishment = self.miscapture_punishment / (self.episode_limit * self.num_predators)
        ...
        self.dropout_prob = getattr(args, "dropout_prob", None)
        ...
        if self.use_adj_matrix:
            if self.dropout:
                if self.dropout_prob is not None:
                    # Dropout mask with dropout probability
                    dropout_mask = np.random.choice([0, 1], size=(self.num_predators, self.num_predators),
                                                    p=[self.dropout_prob, 1-self.dropout_prob])

                    # Keep self-loops or remove them when not used
                    if self.use_self_attention:
                        np.fill_diagonal(dropout_mask, 1)
                    else:
                        np.fill_diagonal(dropout_mask, 0)

                    # Apply dropout mask
                    adj_matrix = self.get_agent_adj_matrix(world) * dropout_mask

                else:
                    # Uniform distribution of zeros and ones
                    dropout_mask = np.random.randint(2, size=(self.num_predators, self.num_predators))
                ...
                adj_matrix = self.get_agent_adj_matrix(world)

            return np.array(obs_n, dtype=np.float32), adj_matrix, np.array(available_actions_n), \
    ...
    def grid_render(self, world, mode='human', save_filename=None, cell_size=35):
        ...
        Returns:
            results: ()
        """
        # if not self.synchronized_predator_prey_scenario:
    ...
    def close_grid_render(self):
        """
        Closes Render window
        """

        self.grid_viewer.close()
